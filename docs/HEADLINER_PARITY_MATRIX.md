...
| Multiple caption sources improvement | Self-hosted app can exceed Headliner by captioning multiple media sources. | A voice-over placed on the clip is transcribed when saved and its words are captioned at its moment, labelled Voice-over, merged by time with the episode's captions. | PASS | Not implemented. | Tests: words land at clip_start + cue time with ids renumbered; saving queues transcription only. |
