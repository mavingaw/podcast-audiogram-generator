...
| Export | Start background render from editor. | Export queues a render and the finished MP4, SRT, VTT and manifest download from the project; a batch downloads as one ZIP. | PASS | `frontend/src/App.tsx`, `backend/app/services/jobs.py` | Driven end to end through the public URL: source → typed clip → Studio → effect → Export → 2.5 MB MP4 fetched. |
...
| Image/logo layers | Images can be placed and replaced. | Any number of image layers: Add to canvas → Media, then pick an uploaded image or upload one for that layer alone. Each is baked as a plate, cropped, optionally rounded, and composited in order. | PASS | Not implemented. | Test: two image layers with real PNGs produce two overlays and two inputs in the graph. |
...
| Multiple caption sources improvement | Self-hosted app can exceed Headliner by captioning multiple media sources. | Captions come from the episode's transcript only. A voice-over placed on the clip is mixed in but not transcribed, so its words are not captioned. Not started; would mean transcribing each recording and merging by time. | FAIL | Not implemented. | Transcribe a placed voice-over and merge its words into the caption timeline. |
...
| Ducking music under speech | Music dips automatically while someone is talking. | Exact when the clip has a transcript: the bed dips by the number on the slider precisely while someone is speaking, driven by word timings, with breaths bridged. Falls back to the sidechain compressor when there is no transcript. | PASS | `backend/app/services/music_bed.py` | Real encode: -12 dB set, measured -12.0 ±0.7 dB inside speech versus outside. |
...
| Voice-over recording | Record narration in the browser. | Record in the browser (MediaRecorder), preview the take, place it at the playhead. Saved as a media asset without the episode's analysis jobs and mixed on export like an effect cue. | PASS | Not implemented. | Real encode with a webm/opus take as a cue: audible at its moment. |
| Per-track volume automation | Adjust a track's level over time. | Music bed level keyframes: park the playhead, Dip / Full / Out; the bed rides between them in straight lines, on top of the automatic ducking. Rendered as a per-frame volume expression. | PASS | Not implemented. | Real encode: a -20 dB dip keyframed over 2.5-3.5 s measures >15 dB quieter inside than outside. |
...
| Back catalog processing | Process prior episodes via project settings. | First sight of a feed takes the newest episode only, on purpose. 'Import older episodes' on a feed pulls the next 1–25 newest not yet imported, each downloading, transcribing and cutting itself into clips; pressing again continues where it left off. | PASS | Not implemented. | Tests: 12-episode feed, three at a time, nothing queued twice, count capped at 50. |
