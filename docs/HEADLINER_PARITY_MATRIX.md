...
| Transcribe and Edit | Transcript-based audio/video editing and generated marketing text. | Local Faster-Whisper with word timings, speaker labels, searchable transcript that drives clip selection, and editable transcript text. Deleting words to cut the audio is still missing. | PARTIAL | `backend/app/services/transcription.py` | `backend/tests/test_transcription.py`; measured at 15x realtime on an RTX 4090. Deleting words to cut media is still absent. |
...
| Volume controls | Adjust source/media volume. | Not available. | FAIL | Not implemented. | Change volume and verify preview/render. |
...
| Effects/transitions | Some assets support effects/transitions. | Not available. | FAIL | Not implemented. | Apply supported effect and verify preview/render. |
| Dynamic RSS elements | Template text/images can be dynamic from RSS metadata. | Placeholder concept in prompt only. | FAIL | Not implemented. | Apply template to episode and verify title/artwork substitution. |
...
| Template timing correctness | Template assets must align with source timing. | No time-aware template model. | FAIL | Not implemented. | Template render with clip and full episode sources. |
...
| Caption timing adjustment | User can adjust caption timing. | Captions are derived from transcript segments only. | FAIL | Not implemented. | Move caption boundary and verify SRT/VTT/render. |
...
| Transcript-based media edits | Document-like deletion can edit corresponding media. | Not available. | FAIL | Not implemented. | Delete words, verify non-destructive edit decision list and render. |
...
| RSS Soundbite tags | Use Soundbite tags when present in RSS feed. | Not available. | FAIL | Not implemented. | Parse soundbite tags and create clips. |
