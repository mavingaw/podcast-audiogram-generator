...
CAPTION_PRESETS = {
    ...
    "frost": {
        "label": "Frost — frosted glass plate",
        # Dark type on a translucent white plate. Over a blurred artwork
        # background this reads as frosted glass; over a flat colour it is a
        # soft light card. The plate alpha is what makes it glass rather
        # than a white box.
        "highlight": BRAND["gold"],
        "size_ratio": 0.068,
        "outline": 3,
...
class RenderLayer:
    ...
    # text layers alike.
    opacity: float = 1.0
    # Text layers only: scales the fitted font size (1.0 = fit the box), and
    # where the text sits inside its box horizontally.
...
class Scene:
    ...
    # Multiplies the preset's caption size. 1.0 is the preset as designed.
...
def parse(
    ...
    return Scene(
        ...
        caption_scale=_clamp(_number(scene.get("captionScale"), 1.0), 0.6, 1.6),
...
def _layer(entry: object, clip_duration: float) -> RenderLayer | None:
    ...
    return RenderLayer(
        ...
        font_scale=_clamp(_number(entry.get("fontScale"), 1.0), 0.5, 1.6),
        align=(
            str(entry.get("align"))
            if entry.get("align") in ("left", "center", "right")
            else "center"
        ),
    )
